...
int main(int argc, char *argv[] )
{
    ...
    cl_kernel box_muller_kernel = clCreateKernel(program, "BoxMuller", &status);
    ...
    for(int j = 0; j < REPEAT ; j++) {
        ...
        free(data);
        printf("passou 1\n");
        cl_mem box_result_buffer =  clCreateBuffer(my_context,
        ...
        printf("passou 2\n");
        //uint *final_size_ptr = (uint *)malloc(sizeof(uint));
        void *final_size_ptr = NULL;
        posix_memalign(&final_size_ptr, AOCL_ALIGNMENT,sizeof(uint));
        cl_mem final_size_buffer = clCreateBuffer(my_context,
                                      CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                                      sizeof(uint) ,
                                      final_size_ptr,
                                      &status);
        ...
        free(output);
        float m = 0; //média
        float s = 1; // sigma - desvio padrão
        

        status = clSetKernelArg(box_muller_kernel, 0, sizeof(cl_mem), &box_result_buffer);
        checkError(status, "Failed to set kernel arg 0");
        status = clSetKernelArg(box_muller_kernel, 1, sizeof(cl_mem), &final_size_buffer);
        checkError(status, "Failed to set kernel arg 1");
        status = clSetKernelArg(box_muller_kernel, 2, sizeof(cl_mem), &input_buffer);
        checkError(status, "Failed to set kernel arg 2");
        status = clSetKernelArg(box_muller_kernel, 3, sizeof(uint), &m);
        checkError(status, "Failed to set kernel arg 3");
        status = clSetKernelArg(box_muller_kernel, 4, sizeof(uint), &s);
        checkError(status, "Failed to set kernel arg 4");
        status = clSetKernelArg(box_muller_kernel, 5, sizeof(int), &size);
        checkError(status, "Failed to set kernel arg 5");
        
        status = clEnqueueNDRangeKernel(queue, box_muller_kernel, 1, NULL, gSize, wgSize, 0, NULL, NULL);
        checkError(status, "Failed to launch kernel");
        printf("passou 4\n");
        status = clFinish(queue);
        void *box_output = NULL;
        posix_memalign(&box_output, AOCL_ALIGNMENT, sizeof(float)*SIZE); // isso é importante
        status = clEnqueueReadBuffer(queue,
                                    box_result_buffer,
                                    CL_TRUE,
                                    0,
        ...
        printf("passou 5\n");                           
        void *size_output = NULL;
        posix_memalign(&size_output, AOCL_ALIGNMENT, sizeof(int)); // isso é importante
        status = clEnqueueReadBuffer(queue,
                                    final_size_buffer,
                                    CL_TRUE,
                                    0,
        ...
        checkError(status, "Read buffer fail");
        printf("passou 6\n");
        int *final_size = (int *)size_output;
        float *data2 = (float *)malloc(sizeof(float)*(*final_size)); //gambiarra para copiar um void em um float
        memcpy(data2,box_output, sizeof(float)*(*final_size));
